// index.js (supports bigger UI + full-height quote + dark/light/auto)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function removeClasses(body, classes) {
        classes.split(" ").forEach((c) => body.classList.remove(c));
    }
    function addClasses(body, classes) {
        classes.split(" ").forEach((c) => body.classList.add(c));
    }
    function clearThemeClasses() {
        const b = document.body;
        Object.values(themeMap).forEach((cls) => removeClasses(b, cls));
    }
    function getSystemPrefersDark() {
        return window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches;
    }
    function effectiveTheme(name) {
        return name === "auto" ? (getSystemPrefersDark() ? "dark" : "light") : name;
    }
    ...
    document.addEventListener("change", (e) => {
        if (e.target.id === "theme-select" || e.target.id === "settings-theme") applyTheme(e.target.value);
    });
    ...
    function animate(p) {
        const pos = reset(p);
        const d = Math.random() * 20 + 15, t = Math.random() * 10;
        setTimeout(() => {
            p.style.transition = `all ${d}s linear`;
            p.style.opacity = Math.random() * 0.15;
            p.style.left = `${pos.x + (Math.random() * 10 - 5)}%`;
            p.style.top = `${pos.y - Math.random() * 20}%`;
            setTimeout(() => animate(p), d * 1000);
        }, t * 1000);
    }
    document.addEventListener("mousemove", (e) => {
        ...
        p.style.width = `${s}px`;
        p.style.height = `${s}px`;
        p.style.left = `${(e.clientX / innerWidth) * 100}%`;
        p.style.top = `${(e.clientY / innerHeight) * 100}%`;
        p.style.opacity = "0.6";
        particlesContainer.appendChild(p);
        setTimeout(() => {
    ...
    function typewriterEffect(el, words, period = 3000) {
        ...
        (function type() {
            const w = words[i];
            if (del) {
                el.textContent = w.substring(0, j - 1);
                j--;
                if (j === 0) {
                    del = false;
                    i = (i + 1) % words.length;
    ...
    function highlightNav(hash) {
        $$(".nav-link").forEach((a) => a.classList.remove("bg-white/15", "text-[1.1rem]"));
        const active = document.querySelector(`a[href="${hash}"]`);
        if (active) active.classList.add("bg-white/15");
    }
    ...
    function renderTasks() {
        ...
        list.forEach((task) => {
            ...
            el.innerHTML = `
                    ...
                    <div class="task-meta">
                        ...
                        ${
                            task.repeat && task.repeat !== "none"
                                ? `<span><i class="fas fa-sync-alt"></i> ${task.repeat}</span>`
                                : ""
                        }
                    </div>
                </div>
                <div class="task-actions">
                    <button class="star-btn" title="Star">${
                        task.star ? '<i class="fas fa-star"></i>' : '<i class="far fa-star"></i>'
                    }</button>
            ...
            el.querySelector(".myday-btn").onclick = () => {
                tasks[index].myDay = !tasks[index].myDay;
                saveTasks();
                renderTasks();
            };
        ...
        if (list.length === 0)
            taskList.innerHTML = `<p style="text-align:center;color:rgba(255,255,255,.5);padding:1.5rem 0;">No tasks to show</p>`;
    }
    ...
    function renderPlanned() {
        ...
        plannedUpcoming.innerHTML =
            up
                .map(
                    (t) =>
                        `<div class="flex items-center justify-between bg-white/5 rounded-lg px-3 py-2"><span>${t.title}</span><span class="text-sm opacity-70">${new Date(
                            t.deadline
                        ).toLocaleString()}</span></div>`
                )
                .join("") || `<p class="opacity-60">Nothing upcoming within a week.</p>`;
        plannedReminders.innerHTML =
            rem
                ...
                .join("") || `<p class="opacity-60">No reminders available.</p>`;
    }
    ...
    async function scheduleReminder(task) {
        ...
        if (timeout > 0) {
            ...
            if (current && !current.completed)
                alert(`Reminder: Your task "${task.title}" is due now!`);
        }
    }
